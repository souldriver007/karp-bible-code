// ============================================================================
// KARP Bible Code — Sharman Family Search
// Run: node scripts/els_family.js
// ============================================================================
...
function elsSearch(stream, term, { minSkip = 1, maxSkip = 3000, direction = 'both' } = {}) {
    ...
    const dirs = [];
    if (direction === 'both' || direction === 'forward') dirs.push(1);
    if (direction === 'both' || direction === 'reverse') dirs.push(-1);
...
async function main() {
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('  KARP Bible Code — SHARMAN FAMILY SEARCH');
    console.log('  Genesis Focus + Torah Expansion');
    console.log('═══════════════════════════════════════════════════════════════');
    ...
    // ═══════════════════════════════════════════════
    console.log(`\n${'═'.repeat(65)}`);
    console.log('  GENESIS — All Family Members (skip 1-3000, both directions)');
    ...
    for (const term of allTerms) {
        ...
        const hits = elsSearch(genesis.stream, term, { maxSkip: 3000 });
    ...
    // ═══════════════════════════════════════════════
    // GENESIS PROXIMITY MATRIX — Every pair
    // ═══════════════════════════════════════════════
    console.log(`\n${'═'.repeat(65)}`);
    console.log('  GENESIS — FAMILY PROXIMITY MATRIX');
    console.log(`${'═'.repeat(65)}`);

    // Check each family member against ADRIAN and SHARMAN
    for (const name of familyNames) {
        ...
        for (const anchor of ['ADRIAN', 'SHARMAN', 'MATZR']) {
            ...
            if (closest) {
                ...
                console.log(`      ${marker} ↔ ${anchor}: ${closest.dist.toLocaleString()} letters apart — ${locA} ↔ ${locB} (skips ${closest.a.skip}/${closest.b.skip})`);
            } else {
                console.log(`      ↔ ${anchor}: no proximity within 10,000`);
            }
        }

        // Also check against other family members
        for (const other of familyNames) {
            ...
            if (closest && closest.dist <= 2000) {
                ...
                console.log(`      ${marker} ↔ ${other}: ${closest.dist.toLocaleString()} letters apart — ${locA} ↔ ${locB}`);
            }
        }
    }
    ...
    console.log('  Looking for the largest family gathering...');
    console.log(`${'═'.repeat(65)}`);

    // Find the region of Genesis with the most family members within 20,000 letters
    const allFamilyHits = [];
    ...
    // Sliding window — find densest cluster
    ...
    if (bestCluster.uniqueNames > 0) {
        ...
        // Show each unique name's best hit in the cluster
    ...
    // ═══════════════════════════════════════════════
    // TORAH SEARCH — expand for terms not found in Genesis
    // ═══════════════════════════════════════════════
    const missingInGenesis = familyNames.filter(n => !genHits[n] || genHits[n].length === 0);

    if (missingInGenesis.length > 0) {
        console.log(`\n${'═'.repeat(65)}`);
        console.log(`  TORAH EXPANSION — Terms not found in Genesis: ${missingInGenesis.join(', ')}`);
        console.log(`${'═'.repeat(65)}`);

        for (const term of missingInGenesis) {
            process.stdout.write(`\n🔎 "${term}" in Torah...`);
            const start = Date.now();
            const hits = elsSearch(torah.stream, term, { maxSkip: 5000 });
            const elapsed = Date.now() - start;
